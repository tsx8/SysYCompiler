...
public class IpsccpPass implements Pass {
    ...
    private void visitInstruction(Instruction inst) {
        if (inst instanceof BinaryInst binary) visitBinary(binary);
        else if (inst instanceof IcmpInst icmp) visitIcmp(icmp);
        else if (inst instanceof PhiInst phi) visitPhi(phi);
        else if (inst instanceof BrInst br) visitBr(br);
        else if (inst instanceof CallInst call) visitCall(call);
        else if (inst instanceof ReturnInst ret) visitReturn(ret);
        else if (inst instanceof ZextInst zext) visitZext(zext);
        else if (inst instanceof LoadInst load) visitLoad(load);
        else {
    ...
    private void visitReturn(ReturnInst ret) {
        ...
        if (!currentRet.equals(newRet)) {
            ...
            for (CallInst call : callSites.get(func)) {
                ssaWorklist.add(call);
            }
        }
    }

    private boolean isPure(Function func) {
        if (purityCache.containsKey(func)) return purityCache.get(func);
        if (visiting.contains(func)) return true;
            ...
            for (Instruction inst : currentBlock.getInstructions()) {
                if (inst instanceof ReturnInst ret) {
                    if (ret.getNumOperands() > 0) {
                        ...
                        if (retVal instanceof ConstInt ci) return ci.getValue();
                ...
                if (inst instanceof BinaryInst bin) {
                    ...
                    ConstInt res = ConstantFolder.foldBinary(bin.getOpCode(), 
                        new ConstInt(IntType.I32, v1), new ConstInt(IntType.I32, v2));
    ...
    private Integer getVal(Value v, Map<Value, Integer> context) {
        if (v instanceof ConstInt ci) return ci.getValue();
        ...
        for (Function func : module.getFunctionList()) {
            ...
            for (BasicBlock bb : func.getBasicBlocks()) {
                ...
                if (last instanceof BrInst br && br.getNumOperands() == 3) {
                    ...
                    if (cond.status == LatticeStatus.CONSTANT) {
                        BasicBlock target = cond.value != 0 ? (BasicBlock) br.getOperand(1) : (BasicBlock) br.getOperand(2);
                        BasicBlock notTaken = cond.value != 0 ? (BasicBlock) br.getOperand(2) : (BasicBlock) br.getOperand(1);
    ...
    private boolean isReadOnly(GlobalVariable gv) {
        for (var use : gv.getUseList()) {
            if (use.user() instanceof StoreInst store) {
                if (store.getOperand(1) == gv) return false;
            }
            if (use.user() instanceof CallInst) return false;
        }
        return true;
    }
}
