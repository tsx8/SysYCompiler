...
find test -name "*.java" > tests.txt && javac -encoding UTF-8 -cp out/classes -d out/tests @tests.txt && java -cp "out/classes:out/tests" top.tsxb.compiler.CompilerTest
```
...
`CompilerConfig` centralizes runtime switches: `CURRENT_HOMEWORK` gates parser/semantic/llvm/mips flows, `OBJECT_CODE` selects IR vs MIPS output, and `OPTIMIZE` enables middle-end passes only—the backend should always emit its best effort. Optimize toward the official score `FinalCycle = DIV*15 + MULT*5 + (JUMP/BRANCH)*2 + MEM*3 + OTHER*1`, prioritizing memory- and branch-reduction even if total instruction count rises slightly.
